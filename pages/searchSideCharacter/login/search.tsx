...
import { Giftbox, ProfileSearchCardSideCharac, BannerBoxConatinerSec } from '../../../components'
...
function Home() {
    ...
    const tabs = [{ key: 1, heading: t('search.searchClient'), content: t('search.searchClient') },
        ...
        <div className="jumbotron">
                        ...
                        <div className={`${styles.pt40}`}>
                            <div className={`${stylessec.pt30} row`}>
                                <span className={`${stylessec.p1}`}>
                                    {t('searchFormSidecharacter.desiredClient')}
                                </span>
                                <div className="row " style={{ width: "100%", paddingTop: "10px", paddingBottom: "30px" }}>
                                    <Radio.Group defaultValue="3">
                                        <Radio.Button value="1" className={`${stylessec.radiobtntxt2}`} style={{ borderTopLeftRadius: "10px", borderBottomLeftRadius: "10px", width: "50%" }}>{t('search.clientbeforeLogin.individual')}</Radio.Button>
                                        <Radio.Button value="3" className={stylessec.radiobtntxt2} style={{ borderBottomRightRadius: "10px", borderTopRightRadius: "10px", width: "50%" }}>{t('search.clientbeforeLogin.company')}</Radio.Button>
                                    </Radio.Group>
                                </div>
                            </div>
                            ...
                            <div className="row" style={{ width: "100%", paddingTop: "8px", paddingBottom: "30px" }}>
            ...
            <div className="h-100 pb-5">
                {!detailedVisible && (<div className="col4-cmn-cntr-800">
                    {activeTab === 0 && profiles.profiles.map((profile, index) => <ProfileSearchCardSideCharac key={index} profile={profile} />)}
                    {activeTab === 1 && profiles.sideCharacprofiles.map((profile, index) => <ProfileSearchCardSideCharac key={index} profile={profile} />)}
                    {activeTab === 2 && profiles.receivedRequests.map((profile, index) => <ProfileSearchCardSideCharac key={index} profile={profile} />)}
                    {activeTab === 3 && profiles.sentRequests.map((profile, index) => <ProfileSearchCardSideCharac key={index} profile={profile} />)}
                    {activeTab === 4 && profiles.likedRequests.map((profile, index) => <ProfileSearchCardSideCharac key={index} profile={profile} />)}
                </div>)}
            </div>
        </div >
    )
